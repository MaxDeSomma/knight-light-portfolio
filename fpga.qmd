...
### boardSPI
`boardSPI` has threee inputs `sck`, `boardState`, and `done`. `sck` is ported from the mcu as the SPI clock. `boardState` comes from `scanningFSM` once it has collected the board date. Lastly `done` is recieved from `scanningFSM` and indicates to this module that `boardState` is stable and ready to sent over to the mcu. There is only on output which is 'sdo'. This module uses a custom SPI protocol similar to lab 7 that shifts out `sdo` one bit at a time on the negative edge of `sck` sending it over bit by bit.